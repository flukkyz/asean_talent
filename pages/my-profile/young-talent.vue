    ...
    <div class="">
      <div class="search-panel py-6">
        <v-container :class="{ 'px-8': ['sm', 'xs'].includes($vuetify.breakpoint.name)}">
          <p>
            Our platform displays information about ASEAN Young Talent, including general profile details, data on research articles published in SCOPUS, ResearchGate, and LinkedIn, to facilitate collaboration between researchers and ASEAN Young Talent.
          </p>
        ...
        <v-container :class="{ 'px-8': ['sm', 'xs'].includes($vuetify.breakpoint.name)}">
          ...
          <div v-if="recommendations && recommendations.length > 0" class="mt-5">
            <p class="title themeAccent--text mb-0">
              Recommended Young talents based on keyword matching
            </p>
            <p>
              This section of the platform recommends Young Talent with research keywords similar to the researcher you are interested in. And you can click on the name of the researcher you are interested in to view their research details.
            </p>
  ...
  async mounted () {
    try {
      const datas = await Promise.all([
        this.$axios.$get(`${this.apiPath}list-keywords-popular?size=10&group=young_talent`),
  ...
  methods: {
    ...
    async showScopusTalentProfile (id) {
      ...
      try {
        ...
        this.$vuetify.goTo(0)
        const recommendations = await this.$axios.$get(`${this.apiPath}${this.api}/${id}/recommendations?size=12&group=young_talent`)
...
.talent-profile {
  background: linear-gradient(to right, #fff7ef 0%, #FFF 100%);
  background-position: top;
  background-repeat: no-repeat;
  background-size: cover;
}
