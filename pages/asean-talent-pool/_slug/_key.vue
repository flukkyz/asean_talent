...
export default {
  ...
  async fetch () {
    ...
    try {
      const datas = await this.$axios.$get(`${this.api}/${this.$route.params.slug}/${this.$route.params.key}?group=talent`)
      this.listDatas = datas
      this.countries = this.$_.uniq(this.listDatas.rows.map(ele => ele.country))
      this.$breadcrumbs.setItems([
        {
          text: 'ASEAN Talent Pool',
          to: { name: 'asean-talent-pool' }
        },
        {
          text: this.listDatas.industry,
          to: { name: 'asean-talent-pool-slug', params: { slug: this.listDatas.industry } }
        },
        {
