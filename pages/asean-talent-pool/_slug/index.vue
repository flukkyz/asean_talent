...
export default {
  ...
  async fetch () {
    ...
    try {
      const dataIndustry = await this.$axios.$get(`${this.apiPath}dashboards/count-talents-group-industry?group=talent&industry=${this.$route.params.slug}&q=${this.search}`)
      this.industry = {
        ...dataIndustry[0],
        page: 0,
        totalPages: 0,
