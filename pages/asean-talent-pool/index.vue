  ...
  <div class="">
    <div class="hero-image" :style="['xs', 'sm'].includes($vuetify.breakpoint.name) ? 'height: 270px;' : 'height: 240px;'">
      <v-container :class="['d-flex align-center h-100',['xs', 'sm'].includes($vuetify.breakpoint.name) ? 'flex-column' : 'justify-space-between px-16']">
        ...
        <img src="/images/ui/search.png" :height="['xs', 'sm'].includes($vuetify.breakpoint.name) ? 152 : 196">
      </v-container>
    </div>
            ...
            <div v-if="data.keywords.length > 0" :key="`domain-industry-${data.domain_industry_id}`" class="mb-8">
              <div class="d-flex align-start">
                <v-card
                  ...
                  :to="localePath({ name: 'asean-talent-pool-slug', params: { slug: data.name } })"
                >
                ...
                <div class="d-flex flex-wrap">
                  ...
                  >
                    <v-chip
                      ...
                      label
                      :to="localePath({ name: 'asean-talent-pool-slug-key', params: { slug: data.name, key: keyword.ListKeyword.keyword } })"
                    >
...
export default {
  ...
  async fetch () {
    ...
    try {
      const datas = await this.$axios.$get(`${this.apiPath}dashboards/count-talents-group-industry?group=talent&q=${this.search}`)
  ...
  beforeMount () {
    if (!this.$store.state.setting.show_menu_asean_talent) {
