<template>
  <v-container>
    <div class="d-flex align-center">
      ...
      <a href="/files/example_young_talent_template.xlsx" target="_blank" rel="Template Excel File">
...
export default {
  ...
  data () {
    return {
      modelName: 'Young Talent Profile',
      modelRoute: 'young-talents',
      sheetUpload: false,
      fileUpload: null,
      search: '',
      listDatas: null,
  ...
  methods: {
    ...
    onAddImages (files) {
      for (const file of files) {
        const findIndex = this.$_.findIndex(this.listDatas, { picture: file.name })
        if (findIndex !== -1) {
    ...
    resetBeforeUpload () {
      this.tableHeaders = this.tableHeaders.slice(0, -1)
