        ...
        <v-card v-if="listDatas">
          <v-card-title class="pb-0">
            ...
            <v-btn
              ...
              depressed
              color="primary"
              @click="$bus.$emit('open-funding-organization-form')"
            >
            ...
            <v-btn
              ...
              depressed
              color="info"
              :to="localePath({name: 'backend-manages-funding-organizations-uploads'})"
            >
              <v-icon small class="mr-1">
                fas fa-upload
              </v-icon>
          ...
          >
            ...
            <template #[`item.edit`]="{ item }">
              <v-icon
                small
                color="warning"
                @click="$bus.$emit('open-funding-organization-form', item)"
              >
                fas fa-edit
              </v-icon>
            </template>
              ...
              <td :colspan="headers.length">
                <div class="pt-3 px-3 pb-6">
                  <div class="d-flex align-center justify-space-between">
                    ...
                    <!-- <div v-if="item.min_budget !== null || item.max_budget !== null" class="text-right"> -->
                    <div v-if="item.budget !== null" class="text-right">
                      <p class="caption grey--text mb-0">
                        Budget
                      </p>
                      <p class="title mb-0 font-wight-bold">
                        <!-- <span v-if="item.min_budget !== null && item.max_budget !== null">
                          {{ $currencyText(item.min_budget) }} -
                          {{ $currencyText(item.max_budget) }}
                        </span>
                        <span v-else-if="item.min_budget !== null">
                          Start {{ $currencyText(item.min_budget) }}
                        </span>
                        <span v-else-if="item.max_budget !== null">
                          In {{ $currencyText(item.min_budget) }}
                        </span> -->
                        {{ $currencyText(item.budget) }}
                        <span v-if="item.Currency" class="body-1">
                          {{ item.Currency.code }}
                        </span>
                      </p>
                    </div>
                  ...
                  <label-value
                    ...
                    :value="$dateText(item.open_date)"
                  />
                  <label-value
                    label="Deadline for apply Date"
                    :value="$dateText(item.deadline_date)"
                  />
                  ...
                  <div v-if="item.eligibility">
                    ...
                    <div v-if="item.eligibility" class="mb-3">
                      <div class="ck-content" v-html="item.eligibility" />
                    </div>
                  </div>
                </div>
              </td>
...
export default {
  ...
  data () {
    return {
      api: `${process.env.apiUrl}${process.env.apiDirectory}funding-organizations`,
