<template>
  <v-container>
    <h3>
      Upload {{ modelName }}
      <a href="/files/example_funding_organization_template.xlsx" target="_blank" rel="Template Excel File">
            ...
            <template #expanded-item="{ headers, item }">
              <td :colspan="headers.length" class="pa-5">
                ...
                <p class="mb-0">
                  Organization
                  &nbsp;
                  <span v-if="item.department" class="font-weight-bold">
                    {{ item.department }}
                  </span>
                ...
                <p class="mb-0">
                  Budget
                  &nbsp;
                  <span v-if="item.budget" class="font-weight-bold">
                    {{ item.budget }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                ...
                <!-- <p class="mb-0">
                  Min budget
                  &nbsp;
                  <span v-if="item.min_budget" class="font-weight-bold">
                    {{ item.min_budget }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                ...
                <p class="mb-0">
                  Max budget
                  &nbsp;
                  <span v-if="item.max_budget" class="font-weight-bold">
                    {{ item.max_budget }}
                  </span>
                  <span v-else>Unknown data</span>
                </p> -->
                ...
                <p class="mb-0">
                  Currency
                  &nbsp;
                  <span v-if="item.currency" class="font-weight-bold">
                    {{ item.currency }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                ...
                <!-- <p class="mb-0">
                  Lat
                  &nbsp;
                  <span v-if="item.lat" class="font-weight-bold">
                    {{ item.lat }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                <p class="mb-0">
                  Lng
                  &nbsp;
                  <span v-if="item.lng" class="font-weight-bold">
                    {{ item.lng }}
                  </span>
                  <span v-else>Unknown data</span>
                </p> -->
                ...
                <p class="mb-0">
                  Call Open Date
                  &nbsp;
                  <span v-if="item.open_date" class="font-weight-bold">
                    {{ $dateText(item.open_date) }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>

                <p class="mb-0">
                  Deadline for apply Dates
                  &nbsp;
                  <span v-if="item.deadline_date" class="font-weight-bold">
                    {{ $dateText(item.deadline_date) }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                ...
                <div class="my-2">
                  ...
                  <p v-if="item.eligibility" class="caption mb-0">
                    {{ item.eligibility }}
                  </p>
                  <span v-else class="grey--text body-2">Unknown data</span>
                </div>
...
export default {
  ...
  data () {
    return {
      ...
      modelRoute: 'funding-organizations',
      sheetUpload: false,
      fileUpload: null,
      search: '',
      listDatas: null,
      ...
      referenceData: {
        ...
        departments: {
          column: 'department',
          api: `${process.env.apiUrl}${process.env.apiDirectory}departments`,
          datas: null,
          newDatas: []
        }
        // countries: {
      ...
      if (this.search) {
        ...
        listFilters = listFilters.filter((item) => {
          return [
            ...
            this.$findSome(search, item.department),
  ...
  async beforeMount () {
    ...
    this.$breadcrumbs.setItems([
      {
        text: this.modelName,
        to: { name: 'backend-manages-funding-organizations' }
      },
      {
        text: 'Upload'
      }
        ...
        for (const data of this.listDatas) {
          ...
          try {
            ...
            if (data.file && data.logo) {
              ...
              formData.append('funding_img', data.file)
