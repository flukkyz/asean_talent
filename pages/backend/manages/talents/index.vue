        ...
        <v-card v-if="listDatas">
          <v-card-title>
            ...
            <v-btn
              ...
              depressed
              color="primary"
              :to="localePath({name: 'backend-manages-talents-form'})"
            >
            ...
            <v-btn
              ...
              depressed
              color="info"
              :to="localePath({name: 'backend-manages-talents-uploads'})"
            >
              <v-icon small class="mr-1">
                fas fa-upload
              </v-icon>
            ...
            <v-btn
              ...
              depressed
              color="info"
              :to="localePath({name: 'backend-manages-talents-update-scopuses'})"
            >
              <v-icon small class="mr-1">
                fas fa-sync
              </v-icon>
          ...
          >
            ...
            <template #[`item.view`]="{ item }">
              <nuxt-link :to="localePath({ name: 'backend-manages-talents-id', params: { id: item.id } })">
                <v-icon
                  small
                >
                  fas fa-list-ul
            ...
            <template #[`item.edit`]="{ item }">
              <nuxt-link :to="localePath({ name: 'backend-manages-talents-form-id', params: { id: item.id } })">
                <v-icon
                  small
                  color="warning"
                >
...
export default {
  ...
  async fetch () {
    ...
    try {
      const datas = await this.$axios.$get(`${this.apiPath}talents${(searchParams ? '?' + searchParams : '')}`)
  ...
  methods: {
    ...
    async submitDelete (id) {
      try {
        await this.$axios.delete(`${this.apiPath}talents/${id}`)
