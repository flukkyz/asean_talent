  ...
  <v-container v-if="data">
    ...
    >
      <v-card-title class="align-start py-3">
        ...
        <v-spacer />
        <nuxt-link class="mr-4" :to="localePath({ name: 'backend-manages-talents-form-id', params: { id: $route.params.id } })">
          <v-icon
            small
            color="warning"
          >
    ...
    <lists-scopus class="mb-5" :talent="data" @refresh="fetchAll" />
  ...
  async fetch () {
    try {
      ...
      if (this.data.Scopus.length > 0) {
        const recommendations = await this.$axios.$get(`${this.apiPath}matchings/${this.data.Scopus[0].id}/recommendations?size=50&group=talent`)
  ...
  methods: {
    ...
    async submitDelete () {
      try {
        ...
        this.$router.push(this.localePath({ name: 'backend-manages-talents' }))
      } catch (e) {
