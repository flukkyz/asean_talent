<template>
  <v-container>
    <div class="d-flex align-center">
      ...
      <a href="/files/example_match_company_template.xlsx" target="_blank" rel="Template Excel File">
            ...
            <template #expanded-item="{ headers, item }">
              <td :colspan="headers.length" class="pa-5">
                ...
                <p class="">
                  Product
                  &nbsp;
                  <span v-if="item.product" class="font-weight-bold">
                    {{ item.product }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                ...
                <p class="mb-0">
                  Size of Company
                  &nbsp;
                  <span v-if="item.size" class="font-weight-bold">
                    {{ item.size }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                ...
                <p class="mb-0">
                  Address
                  &nbsp;
                  <span v-if="item.address" class="font-weight-bold">
                    {{ item.address }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
...
export default {
      ...
      if (this.search) {
        ...
        listFilters = listFilters.filter((item) => {
          return [
            ...
            this.$findSome(search, item.product),
            this.$findSome(search, item.contact_person)
          ].some(ele => ele)
        })
      }
      return listFilters
  ...
  async beforeMount () {
    ...
    this.$breadcrumbs.setItems([
      {
        text: this.modelName,
        to: { name: 'backend-manages-match-companies' }
      },
      {
        text: 'Upload'
      }
        ...
        for (const data of this.listDatas) {
          ...
          try {
            ...
            if (data.file && data.logo) {
              ...
              formData.append('company_img', data.file)
