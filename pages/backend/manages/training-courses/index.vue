        ...
        <v-card v-if="listDatas">
          <v-card-title class="pb-0">
            ...
            <v-btn
              ...
              depressed
              color="primary"
              @click="$bus.$emit('open-training-course-form')"
            >
            ...
            <v-btn
              ...
              depressed
              color="info"
              :to="localePath({name: 'backend-manages-training-courses-uploads'})"
            >
              <v-icon small class="mr-1">
                fas fa-upload
              </v-icon>
          ...
          >
            ...
            <template #[`item.format`]="{ item }">
              {{ trainingFormats[item.format] }}
            </template>
            <template #[`item.start_date`]="{ item }">
              {{ $dateText(item.start_date,'medium','medium') }}
            </template>
            ...
            <template #[`item.edit`]="{ item }">
              <v-icon
                small
                color="warning"
                @click="$bus.$emit('open-training-course-form', item)"
              >
                fas fa-edit
              </v-icon>
            </template>
              ...
              <td :colspan="headers.length">
                <div class="pt-3 px-3 pb-6">
                  <div class="d-flex align-center justify-space-between">
                    ...
                    <div v-if="item.cost" class="text-right">
                      <p class="caption grey--text mb-0">
                        Cost
                      </p>
                      <p class="title font-weight-bold primary--text grey--text mb-0">
                        {{ $currencyText(item.cost) }}
                  ...
                  <label-value
                    label="Format"
                    :value="trainingFormats[item.format]"
                  />
                  ...
                  <label-value
                    label="Start Date"
                    :value="$dateText(item.start_date,'medium','medium')"
                  />
                  <div v-if="item.detail_date" class="pl-16 mb-3">
                    <div class="pl-16">
                      <div class="pl-16">
                        <div class="pl-4">
                  ...
                  />
                  <div v-if="item.place" class="pl-16 mb-3">
                    <div class="pl-16">
                      <div class="pl-16">
                        <div class="pl-4">
                          <p class="caption mb-0">
...
export default {
  ...
  data () {
    return {
      api: `${process.env.apiUrl}${process.env.apiDirectory}training-courses`,
