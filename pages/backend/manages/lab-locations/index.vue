<template>
  <v-container>
        ...
        <v-card v-if="listDatas">
          <v-card-title class="pb-0">
            ...
            <v-btn
              ...
              depressed
              color="primary"
              @click="$bus.$emit('open-lab-location-form')"
            >
            ...
            <v-btn
              ...
              depressed
              color="info"
              :to="localePath({name: 'backend-manages-lab-locations-uploads'})"
            >
              <v-icon small class="mr-1">
                fas fa-upload
              </v-icon>
          ...
          >
            ...
            <template #[`item.edit`]="{ item }">
              <v-icon
                small
                color="warning"
                @click="$bus.$emit('open-lab-location-form', item)"
              >
                fas fa-edit
              </v-icon>
            </template>
            ...
            <template #expanded-item="{ headers, item }">
              <td :colspan="headers.length">
                <div class="pt-3 px-3 pb-6">
                  ...
                  <label-value
                    label="Organization"
                    :value="item.University.name"
                  />

                  <label-value
                    label="Lab Link"
                  >
                    <a v-if="item.lab_link" :href="item.lab_link" target="_blank" :rel="item.name">
                      {{ item.lab_link }}
                    </a>
                  ...
                  <div class="d-flex align-center  mt-5">
                    ...
                    <v-btn
                      ...
                      depressed
                      color="primary"
                      @click="$bus.$emit('open-machine-detail-form',item)"
                    >
                      <v-icon small class="mr-1">
                        fas fa-plus
                      </v-icon>
                    ...
                    <template #default>
                      <thead>
                        <tr>
                          ...
                          <th class="text-center font-weight-bold" width="50">
                            Edit
                          </th>
                          <th class="text-center font-weight-bold" width="50">
                            Delete
                      ...
                      <tbody>
                        <tr
                          v-for="(machineDetail,index) in item.MachineDetails"
                        ...
                        >
                          ...
                          <td class="text-right">
                            {{ machineDetail.price !== null ? $currencyText(machineDetail.price) : '-' }}
                          </td>
                          <td class="text-left">
                            {{ machineDetail.link }}
                          </td>
                          <td class="text-right">
                            {{ machineDetail.service_condition !== null ? $currencyText(machineDetail.service_condition) : '-' }}
                          </td>
                          <td class="text-left">
                            {{ machineDetail.Currency ? machineDetail.Currency.code : '-' }}
                          </td>
                          ...
                          <td class="text-center">
                            <v-icon
                              small
                              color="warning"
                              @click="$bus.$emit('open-machine-detail-form', item,machineDetail)"
                            >
                              fas fa-edit
                            </v-icon>
                          </td>
                          <td class="text-center">
                            <v-icon
                              small
                              color="error"
                              @click="$bus.$emit('open-delete-dialog-machine-detail', machineDetail.id, `Equipment:  ${machineDetail.name}`)"
                            >
                              fas fa-trash
                            </v-icon>
                          </td>
    ...
    />
    <forms-machine-detail
      @add="submitCreateMachineDetail"
      @edit="submitUpdateMachineDetail"
    />
    <forms-expired
      @save="submitExpired"
    />
    <dialogs-delete @delete="submitDelete" />
    <dialogs-delete value="machine-detail" @delete="submitDeleteMachineDetail" />
  </v-container>
</template>
...
export default {
  ...
  data () {
    return {
      api: `${process.env.apiUrl}${process.env.apiDirectory}lab-locations`,
      apiMachineDetail: `${process.env.apiUrl}${process.env.apiDirectory}machine-details`,
      modelName: 'Lab Location',
      listDatas: null,
      queryParams: {
        size: 20,
        q: '',
  ...
  methods: {
    ...
    async submitCreateMachineDetail (data) {
      try {
        const result = await this.$axios.$post(`${this.apiMachineDetail}`, data)
        await this.$fetch()
        if (result) {
          this.$notifier.showMessage({ title: 'Created', content: 'Create Equipment Successfully', color: 'success' })
        }
      } catch (e) {
        this.$notifier.showMessage({ title: 'Error', content: e, color: 'error' })
      }
    ...
    async submitUpdateMachineDetail (data) {
      try {
        const result = await this.$axios.$put(`${this.apiMachineDetail}/${data.id}`, data)
        await this.$fetch()
        if (result) {
          this.$notifier.showMessage({ title: 'Updated', content: 'Updated Equipment Successfully', color: 'success' })
        }
      } catch (e) {
        this.$notifier.showMessage({ title: 'Error', content: e, color: 'error' })
      }
    ...
    async submitDeleteMachineDetail (id) {
      try {
        await this.$axios.delete(`${this.apiMachineDetail}/${id}`)
        this.$notifier.showMessage({ title: 'Deleted', content: 'Deleted Equipment Successfully', color: 'success' })
        this.$fetch()
      } catch (e) {
        this.$notifier.showMessage({ title: 'Error', content: e, color: 'error' })
      }
