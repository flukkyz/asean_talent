<template>
  <v-container>
    <h3>
      Upload {{ modelName }}
      <a href="/files/example_lab_location_template.xlsx" target="_blank" rel="Template Excel File">
            ...
            <template #expanded-item="{ headers, item }">
              <td :colspan="headers.length" class="pa-5">
                ...
                <p class="mb-0">
                  Organization
                  &nbsp;
                  <span v-if="item.org_name" class="font-weight-bold">
                    {{ item.org_name }}
                  </span>
                ...
                <p class="mb-0">
                  Machine Name
                  &nbsp;
                  <span v-if="item.machine_name" class="font-weight-bold">
                    {{ item.machine_name }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                ...
                <p class="mb-0">
                  Machine Status
                  &nbsp;
                  <span v-if="item.machine_status" class="font-weight-bold">
                    {{ item.machine_status }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                ...
                <p class="mb-0">
                  Machine Price
                  &nbsp;
                  <span v-if="item.machine_price" class="font-weight-bold">
                    {{ item.machine_price }}
                  </span>
                  <span v-else>Unknown data</span>
                </p>
                ...
                <p class="mb-0">
                  ...
                  <span class="font-weight-bold">
                    <a
                      v-if="item.machine_link"
                      :href="item.machine_link"
                      target="_blank"
                    >
                      {{ item.machine_link }}
                    </a>
                ...
                <p class="mb-0">
                  Service Condition
                  &nbsp;
                  <span v-if="item.service_condition" class="font-weight-bold">
...
export default {
      ...
      if (this.search) {
        ...
        listFilters = listFilters.filter((item) => {
          return [
            ...
            this.$findSome(search, item.org_name),
  ...
  async beforeMount () {
    ...
    this.$breadcrumbs.setItems([
      {
        text: this.modelName,
        to: { name: 'backend-manages-lab-locations' }
      },
      {
        text: 'Upload'
      }
        ...
        for (const data of this.listDatas) {
          ...
          try {
            ...
            if (data.file && data.logo) {
              ...
              formData.append('lab_img', data.file)
