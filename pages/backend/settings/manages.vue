        ...
        <v-card v-if="listDatas">
          <v-card-title class="pb-0">
            ...
            <v-btn
              ...
              depressed
              color="primary"
              @click="$bus.$emit('open-manage-form')"
            >
          ...
          <v-card-title class="py-0">
            <p class="caption info--text mb-0 wrap-word">
              Manage logos or names of organizations that manage or operate the platform. You can add, edit, show/hide, or delete management information. Only active items will appear on the website.
            </p>
          </v-card-title>
          ...
          >
            ...
            <template #[`item.edit`]="{ item }">
              <v-icon
                small
                color="warning"
                @click="$bus.$emit('open-manage-form', item)"
              >
                fas fa-edit
              </v-icon>
            </template>
...
export default {
  ...
  data () {
    return {
      api: `${process.env.apiUrl}${process.env.apiDirectory}manages`,
      modelName: 'Managed by',
      listDatas: null,
      queryParams: {
        size: 20,
        q: '',
