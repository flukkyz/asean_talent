<template>
  <div v-if="listDatas" class="pb-16">
    <div class="d-flex mb-3">
      ...
      <v-spacer />
      <v-btn v-if="listDatas.length > 0 && listDatas.length <2" small color="primary" @click="$bus.$emit('open-image-form','banner_img')">
        <v-icon small left>
          mdi-plus
        </v-icon>
        {{ modelName }}
    ...
    <div v-else>
      <v-card
        elevation="0"
        @click="$bus.$emit('open-image-form','banner_img')"
      >
        <v-card-text>
...
export default {
  ...
  data () {
    return {
      modelName: 'Banner',
      api: `${process.env.apiUrl}${process.env.apiDirectory}banners`,
      listDatas: null
    }
  },
  async fetch () {
