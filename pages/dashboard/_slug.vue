...
export default {
  ...
  async fetch () {
    ...
    try {
      let url = `${this.api}/countries/${this.$route.params.slug}?group=talent`
      if (this.$route.query && this.$route.query.industry) {
        url += `&industry=${this.$route.query.industry}`
      }
      const datas = await this.$axios.$get(url)
      ...
      this.$breadcrumbs.setItems([
        ...
        {
          text: `Country : ${this.listDatas.country.name}${this.listDatas.domainIndustry ? ` : ${this.listDatas.domainIndustry.name}` : ''}`
        }
      ])
