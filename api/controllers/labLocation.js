...
const LabLocation = db.LabLocation
...
module.exports = {
  inputValidate: async (req, res, next) => {
    const v = new Validator(req.body, LabLocation.inputSchema)
  ...
  count: async (req, res, next) => {
    try {
      const data = await LabLocation.count()
  ...
  index: async (req, res, next) => {
    ...
    try {
      const lists = await LabLocation.findAndCountAll({
        include: [
          City,
          Country,
          University,
  ...
  store: async (req, res, next) => {
    ...
    try {
      const newData = await db.sequelize.transaction(async (t) => {
        ...
        return LabLocation.create(data, {
          include: [
            City,
            Country,
            University,
  ...
  storeList: async (req, res, next) => {
    ...
    try {
      await db.sequelize.transaction((t) => {
        return LabLocation.bulkCreate(data.lists, {
          transaction: t
        })
      })
  ...
  show: async (req, res, next) => {
    ...
    try {
      const data = await model.findByPk(LabLocation, id, res, [City, Country, University, OrganizationType, Img, {
        model: MachineDetail,
        include: [Currency],
        order: [
          ['price', 'desc'],
  ...
  update: async (req, res, next) => {
    const id = req.params.id
    const data = req.body
    const oldData = await model.findByPk(LabLocation, id, res)
  ...
  removeImage: async (req, res, next) => {
    ...
    try {
      ...
      oldImg.destroy()
      await db.sequelize.transaction(async (t) => {
        return await LabLocation.update(
          { img_id: null }, {
            where: {
              id
            }
