...
module.exports = {
  inputValidate: async (req, res, next) => {
    const v = new Validator(req.body, Manage.inputSchema)
  ...
  index: async (req, res, next) => {
    ...
    try {
      const lists = await Manage.findAndCountAll({
        include: [
          Img
        ],
        where,
  ...
  update: async (req, res, next) => {
    ...
    const data = req.body
    const oldData = await model.findByPk(Manage, id, res)
    try {
      let oldImg
