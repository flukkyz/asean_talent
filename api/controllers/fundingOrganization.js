...
const FundingOrganization = db.FundingOrganization
...
module.exports = {
  inputValidate: async (req, res, next) => {
    const v = new Validator(req.body, FundingOrganization.inputSchema)
  ...
  count: async (req, res, next) => {
    try {
      const data = await FundingOrganization.count()
  ...
  index: async (req, res, next) => {
    ...
    try {
      const lists = await FundingOrganization.findAndCountAll({
        include: [
          Department,
          City,
          Country,
  ...
  store: async (req, res, next) => {
    ...
    try {
      const newData = await db.sequelize.transaction(async (t) => {
        ...
        return FundingOrganization.create(data, {
          include: [
            Department,
            City,
            Country,
  ...
  storeList: async (req, res, next) => {
    ...
    try {
      await db.sequelize.transaction((t) => {
        return FundingOrganization.bulkCreate(data.lists, {
          transaction: t
        })
      })
  ...
  show: async (req, res, next) => {
    ...
    try {
      const data = await model.findByPk(FundingOrganization, id, res, [Department, City, Country, Currency, OrganizationType, Img], null, { active: true })
      return res.json(data)
    } catch (e) {
      e.message = 'Cannot get data from database. Error: ' + e
      next(e)
  ...
  update: async (req, res, next) => {
    const id = req.params.id
    const data = req.body
    const oldData = await model.findByPk(FundingOrganization, id, res)
    ...
    try {
      ...
      await db.sequelize.transaction(async (t) => {
        ...
        return FundingOrganization.update(
          data, {
            where: {
              id
            }
  ...
  removeImage: async (req, res, next) => {
    ...
    try {
      ...
      oldImg.destroy()
      await db.sequelize.transaction(async (t) => {
        return await FundingOrganization.update(
          { img_id: null }, {
            where: {
              id
            }
