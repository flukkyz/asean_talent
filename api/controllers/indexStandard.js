...
module.exports = {
  inputValidate: async (req, res, next) => {
    const v = new Validator(req.body, IndexStandard.inputSchema)
  ...
  count: async (req, res, next) => {
    try {
      const data = await IndexStandard.count()
  ...
  index: async (req, res, next) => {
    ...
    try {
      const lists = await IndexStandard.findAndCountAll({
        where,
        limit,
        offset,
        distinct: true,
  ...
  store: async (req, res, next) => {
    ...
    try {
      const newData = await db.sequelize.transaction((t) => {
        return IndexStandard.create(data, {
          transaction: t
        })
      })
  ...
  storeList: async (req, res, next) => {
    ...
    try {
      await db.sequelize.transaction((t) => {
        return IndexStandard.bulkCreate(data.lists, {
          transaction: t
        })
      })
  ...
  show: async (req, res, next) => {
    ...
    try {
      const data = await model.findByPk(IndexStandard, id, res)
      return res.json(data)
    } catch (e) {
      e.message = 'Cannot get data from database. Error: ' + e
      next(e)
  ...
  destroy: async (req, res, next) => {
    const id = req.params.id
    const oldData = await model.findByPk(IndexStandard, id, res)
    try {
