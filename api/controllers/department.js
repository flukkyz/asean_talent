...
module.exports = {
  inputValidate: async (req, res, next) => {
    const v = new Validator(req.body, Department.inputSchema)
  ...
  count: async (req, res, next) => {
    try {
      const data = await Department.count()
  ...
  index: async (req, res, next) => {
    ...
    try {
      const lists = await Department.findAndCountAll({
        where,
        limit,
        offset,
        distinct: true,
  ...
  indexByFunding: async (req, res, next) => {
    ...
    const query = await db.sequelize.query('SELECT DISTINCT department_id FROM funding_organizations', {
      type: QueryTypes.SELECT
    })
    let where = {
      id: {
        [Op.in]: query.map(ele => ele.department_id)
      }
    }
  ...
  storeList: async (req, res, next) => {
    ...
    try {
      await db.sequelize.transaction((t) => {
        return Department.bulkCreate(data.lists, {
          transaction: t
        })
      })
  ...
  show: async (req, res, next) => {
    ...
    try {
      const data = await model.findByPk(Department, id, res)
      return res.json(data)
    } catch (e) {
      e.message = 'Cannot get data from database. Error: ' + e
      next(e)
  ...
  destroy: async (req, res, next) => {
    const id = req.params.id
    const oldData = await model.findByPk(Department, id, res)
    try {
