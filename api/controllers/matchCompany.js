...
const MatchCompany = db.MatchCompany
...
module.exports = {
  inputValidate: async (req, res, next) => {
    const v = new Validator(req.body, MatchCompany.inputSchema)
  ...
  count: async (req, res, next) => {
    try {
      const data = await MatchCompany.count()
  ...
  index: async (req, res, next) => {
    ...
    try {
      const lists = await MatchCompany.findAndCountAll({
        include: [
          City,
          Country,
          OrganizationType,
  ...
  store: async (req, res, next) => {
    ...
    try {
      const newData = await db.sequelize.transaction(async (t) => {
        ...
        return MatchCompany.create(data, {
          include: [
            City,
            Country,
            OrganizationType,
  ...
  storeList: async (req, res, next) => {
    ...
    try {
      await db.sequelize.transaction((t) => {
        return MatchCompany.bulkCreate(data.lists, {
          transaction: t
        })
      })
  ...
  show: async (req, res, next) => {
    ...
    try {
      const data = await model.findByPk(MatchCompany, id, res, [City, Country, OrganizationType, Img],
        null, { active: true }
      )
  ...
  update: async (req, res, next) => {
    const id = req.params.id
    const data = req.body
    const oldData = await model.findByPk(MatchCompany, id, res)
  ...
  removeImage: async (req, res, next) => {
    ...
    try {
      ...
      oldImg.destroy()
      await db.sequelize.transaction(async (t) => {
        return await MatchCompany.update(
          { img_id: null }, {
            where: {
              id
            }
