...
module.exports = {
  inputValidate: async (req, res, next) => {
    const v = new Validator(req.body, Collaboration.inputSchema)
  ...
  count: async (req, res, next) => {
    try {
      const data = await Collaboration.count()
  ...
  index: async (req, res, next) => {
    ...
    try {
      const lists = await Collaboration.findAndCountAll({
        limit,
        offset,
        distinct: true,
        order: [
  ...
  storeList: async (req, res, next) => {
    ...
    try {
      await db.sequelize.transaction((t) => {
        return Collaboration.bulkCreate(data.lists, {
          transaction: t
        })
      })
  ...
  show: async (req, res, next) => {
    ...
    try {
      const data = await model.findByPk(Collaboration, id, res)
      return res.json(data)
    } catch (e) {
      e.message = 'Cannot get data from database. Error: ' + e
      next(e)
  ...
  destroy: async (req, res, next) => {
    const id = req.params.id
    const oldData = await model.findByPk(Collaboration, id, res)
    try {
