...
module.exports = {
  inputValidate: async (req, res, next) => {
    const v = new Validator(req.body, BlogCategory.inputSchema)
  ...
  count: async (req, res, next) => {
    try {
      const data = await BlogCategory.count()
  ...
  index: async (req, res, next) => {
    ...
    try {
      const lists = await BlogCategory.findAndCountAll({
        where,
        limit,
        offset,
        distinct: true,
  ...
  storeList: async (req, res, next) => {
    ...
    try {
      await db.sequelize.transaction((t) => {
        return BlogCategory.bulkCreate(data.lists, {
          transaction: t
        })
      })
  ...
  show: async (req, res, next) => {
    ...
    try {
      const data = await model.findByPk(BlogCategory, id, res)
      return res.json(data)
    } catch (e) {
      e.message = 'Cannot get data from database. Error: ' + e
      next(e)
  ...
  destroy: async (req, res, next) => {
    const id = req.params.id
    const oldData = await model.findByPk(BlogCategory, id, res)
    try {
