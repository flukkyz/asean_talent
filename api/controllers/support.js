...
module.exports = {
  inputValidate: async (req, res, next) => {
    const v = new Validator(req.body, Support.inputSchema)
  ...
  index: async (req, res, next) => {
    ...
    try {
      const lists = await Support.findAndCountAll({
        include: [
          Img
        ],
        where,
  ...
  update: async (req, res, next) => {
    ...
    const data = req.body
    const oldData = await model.findByPk(Support, id, res)
    try {
      let oldImg
