...
export default {
  ...
  created () {
    this.$bus.$on('open-lab-location-form', async (data) => {
  ...
  beforeDestroy () {
    this.$bus.$off('open-lab-location-form')
  },
  methods: {
    async fetchRefs () {
      await Promise.all([
    ...
    changeImg (val) {
      if (val) {
        this.imgPreview = URL.createObjectURL(this.form.lab_img)
      }
    },
    closeDialog () {
      this.dialog = false
