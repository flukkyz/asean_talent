...
export default {
  ...
  created () {
    this.$bus.$on('open-manage-form', (data) => {
  ...
  beforeDestroy () {
    this.$bus.$off('open-manage-form')
  },
  methods: {
    changeImg (val) {
      if (val) {
        console.log(this.form.manage_img)
        this.imgPreview = URL.createObjectURL(this.form.manage_img)
      }
    },
    closeDialog () {
      this.dialog = false
