...
export default {
  ...
  created () {
    this.$bus.$on('open-host-form', async (data) => {
  ...
  methods: {
    ...
    changeImg (val) {
      if (val) {
        this.imgPreview = URL.createObjectURL(this.form.host_img)
      }
    },
    closeDialog () {
      this.dialog = false
