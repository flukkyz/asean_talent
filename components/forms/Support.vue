...
export default {
  ...
  created () {
    this.$bus.$on('open-support-form', (data) => {
  ...
  beforeDestroy () {
    this.$bus.$off('open-support-form')
  },
  methods: {
    changeImg (val) {
      if (val) {
        console.log(this.form.support_img)
        this.imgPreview = URL.createObjectURL(this.form.support_img)
      }
    },
    closeDialog () {
      this.dialog = false
